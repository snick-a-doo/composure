...
/// @return The index of a pitch picked at random, weighted by dissonance.
int pick_discord(const Vd& pitch)
{
    auto local_discord = [&pitch](double p, std::size_t i) {
        int sum = 0;
        for (size_t j = 0; j < pitch.size(); ++j)
        {
            if (j == i)
                continue;
            sum += dissonance(static_cast<int>(p), static_cast<int>(pitch[j]));
        }
        return sum;
    };

    Vi weight(pitch.size());
    for (std::size_t i = 0; i < pitch.size(); ++i)
        weight[i] = local_discord(pitch[i], i);
    return pick(weight);
}
}

Phrase set_notes(Phrase phrase, double duration, double volume, const Vd& pitch, double delta_t)
{
    const auto voices = pitch.size();
    double t = phrase.end_time;
    for (std::size_t j = 0; j < voices; j++)
        phrase.notes.push_back({t + j*delta_t, duration, volume, pitch[j]});
    phrase.end_time = t + duration;
    return phrase;
}
...
Phrase walk(Phrase phrase, int tonic, int voices, int max_range)
{
    ...
    if (phrase.notes.size() >= static_cast<size_t>(voices))
        for (int i = 0; i < voices; ++i)
            pitch[i] = phrase.notes[phrase.notes.size() - voices + i].pitch;
    ...
    for (double span = 0.0; span < max_range && phrase.notes.size() < 1000; span = range(pitch))
    {
        // Move the "most discordant" note by -2, -1, 0, 1, or 2 scale degrees.
        auto move_idx = pick_discord(pitch);
        ...
        phrase = set_notes(phrase, dur, 0.1, pitch, -(dur*(voices+1))/voices);
...
std::vector<Phrase> filter(Phrase phrase)
{
    ...
    // Sort notes by time.
    std::sort(phrase.notes.begin(), phrase.notes.end(),
    ...
    for (std::size_t ip = 0; ip < poi.size(); ++ip)
    {
        ...
        {
            auto index = poi[ip] + jp;
            part.notes.push_back(phrase.notes[index]);
        }
        part.end_time = phrase.notes[poi[ip] + bin].time;
...
std::string score(const Phrase& phrase, const Vd& partials, double tempo)
{
    ...
    for (const auto& p : partials)
        out << ' ' << p;
    out << std::endl;
    out << score(phrase, tempo);
    return out.str();
}

std::string score(const Phrase& phrase, double tempo)
{
    std::ostringstream out;
    if (tempo > 0.0)
